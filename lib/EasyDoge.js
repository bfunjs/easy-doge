...
function (_Component) {
  ...
  function EasyDoge() {
    ...
    _this.getDoge = function (child) {
      ...
      var _ref = _this.state.dragging >= 0 ? _this.newLayout[child.key] : _this.layoutMap[child.key],
          x = _ref.x,
          y = _ref.y,
          index = _ref.index,
          fixed = _ref.fixed;
    ...
    _this.onDragStop = function () {
      ...
      _this.layoutMap = Object.keys(_this.layoutMap).map(function (key) {
  ...
  _createClass(EasyDoge, [{
    ...
    key: "initLayoutMap",
    value: function initLayoutMap() {
      var _this2 = this;

      var _this$props3 = this.props,
          col = _this$props3.col,
          fixedKeys = _this$props3.fixedKeys;
      this.layoutMap = {};

      _react["default"].Children.map(this.props.children, function (child, index) {
        var fixed = fixedKeys.indexOf(child.key) >= 0;
        _this2.layoutMap[child.key] = {
          x: index % col,
    ...
    value: function render() {
      ...
      var totalHeight = (height + margin[0]) * (Math.floor((Object.keys(this.layoutMap).length - 1) / col) + 1);
